...
class model3_neural(torch.nn.Module):
    def __init__(self, item_num,args):
        ...
        self.generate_layer1 = torch.nn.Linear(32, 32, bias=False)
        self.relu1 = nn.ReLU()
        self.generate_layer2 = torch.nn.Linear(32, 32, bias=True)
        ...
        self.meta_optim = torch.optim.SGD([{'params': self.generate_layer1.parameters(), 'lr': self.global_lr },
                                           {'params': self.generate_layer2.weight, 'lr': self.global_lr},
                                           {'params': self.generate_layer2.bias, 'lr': self.global_lr*0.1}
                                           ])
    ...
    def evaluate(self, train_data, val_negative_dict, val_negative):
        ...
        if self.args.mod=='train' or self.args.mod=='test':
            idxs = random.sample(range(len(train_data.support_fill)), 1000)
        else:
            idxs = range(train_data.idx)
        for idx in idxs:
