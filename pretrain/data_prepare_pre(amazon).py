...
class Test_data(data.Dataset):
    ...
    def ng_test_sample(self):
        ...
        for (u, tmp_item_list) in test_support_data.items():
            ...
            test_support_fill[idx]['user_list'] = user_list
            test_support_fill[idx]['item_list'] = item_list
            test_support_fill[idx]['label_list'] = label_list
            idx+=1
        self.idx = idx
    ...
    def get_batch(self,idxs):
        ...
        label_batch = []
        for idx in idxs:
            user_batch.extend(self.test_support_fill[idx]['user_list'])
            item_batch.extend(self.test_support_fill[idx]['item_list'])
            label_batch.extend(self.test_support_fill[idx]['label_list'])
