...
class PMFLoss(torch.nn.Module):
    def __init__(self, user_num,item_num,lam_u=0.01):
        super().__init__()
        self.lam_u = lam_u
        ...
        self.optim = optim.Adam([self.user_embedding, self.item_embedding], lr=0.0005)
...
if __name__=='__main__':
    ...
    Train_data = data_prepare.Test_data(train_data, item_num, train_mat, 3)
    Train_data.ng_test_sample()

    hrmax = 0.0
    ndcgmax = 0.0
    for epoch in range(1, 100):
        sampler = data.BatchSampler(data.RandomSampler(range(Train_data.idx)), batch_size=256, drop_last=False)
        pmfloss.train()
        loss_sum = 0.0
